...
new URLSearchParams(window.location.hash.replace(/^#\?/, '')).get('bg') ||
'dark';

return (
<style>{`      .sbdocs, .sbdocs-content { 
        padding: 0 !important; 
        margin: 0 !important; 
        background: ${bg === 'white' || bg === 'light' ? '#ffffff' : 'transparent'} !important; 
      }
      body { 
        background: ${bg === 'white' || bg === 'light' ? '#ffffff !important' : 'inherit'};
      }
      html { 
        background: ${bg === 'white' || bg === 'light' ? '#ffffff !important' : 'inherit'};
      }
      ...
      .code-status { 
        ...
        color: ${bg === 'white' || bg === 'light' ? '#333' : '#666'}; 
        padding: 8px 0; 
      }
